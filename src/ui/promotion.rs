...
impl<'ttf_module, 'rwops> PromotionRenderer<'ttf_module, 'rwops> {
    pub fn new(render_area: Rect, chess_renderer: &'ttf_module FontRenderer<'ttf_module, 'rwops>, text_renderer: &'ttf_module FontRenderer<'ttf_module, 'rwops>) -> Self {
        Self {
            render_area,
            chess_renderer,
            text_renderer
...
impl<'ttf_module, 'rwops> PromotionRenderer<'ttf_module, 'rwops> {
    pub fn render(
        ...
        for (tile, p) in self.tiles(options)
        {
            ...
            self.chess_renderer.render_at(p.kind.to_ttf(p.color), tile, canvas)?;
        };

        self.text_renderer.render_at("Promote Pawn", Rect::new((SCREEN_WIDTH - 200) as i32 / 2,100, 200, 100), canvas)?;
        Ok(())
    }
    ...
    fn tiles(&self, options: &Vec<Piece>) -> Vec<(Rect, Piece)> {
        ...
        options
            .iter()
            .enumerate()
            .map(|(idx, piece)| (Rect::new(offset_x + (idx as i32 * width), offset_y, width as u32, width as u32), *piece))
            .collect()
    }
    ...
    fn is_point_in_rect(mouse_x: i32, mouse_y: i32, tile: Rect) -> bool {
        mouse_x > tile.x && mouse_x < tile.x + tile.w && mouse_y > tile.y && mouse_y < tile.y + tile.h
    }
}
